...
def RunSeed(seed,key):
    ...
    if dataset == "SyGuS":
        c_num=3
    train_result_file = f"output/train_result_{key}_4_{c_num}_{seed}.json"
    with open(train_result_file,'r', encoding='UTF-8') as f:
        train_result = json.load(f)
    actual_center_count = len(train_result['center']['center'])
    test_result_file = f"output/test_result_{key}_{seed}_{actual_center_count}.json"
