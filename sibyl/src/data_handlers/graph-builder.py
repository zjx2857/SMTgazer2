...
def build_graph(file):
    # PySMT otherwise falls back to a process-global Environment.  Graph-cache
    # workers parse many unrelated queries in the same process, and common
    # SyGuS names such as `inv-f` legitimately have different signatures in
    # different files.  A fresh environment per query prevents one file's
    # symbol table and formula-manager memoization from contaminating the next.
    environment = Environment()
    parser = SmtLibParser(environment=environment)
    with open(file, "r", encoding="utf-8") as query_file:
        formula = parser.get_script(query_file).get_last_formula()
    if formula is None:
        raise ValueError(f"SMT query has no formula: {file}")

    astBuilder = ASTBuilder(env=environment)
...
def main(parser):
    ...
    file = args.file[0]
    nodes, edges, edge_attr = build_graph(file)
