...
class GAT(torch.nn.Module):
    def __init__(self, passes, inputLayerSize, outputLayerSize, numAttentionLayers, mode, pool, k, dropout, shouldJump=True):
        ...
        if pool == "add":
            ...
            self.pool = AttentionalAggregation(gate_nn=nn.Sequential(torch.nn.Linear(fcInputLayerSize-1, 1), nn.LeakyReLU()))
        elif pool == "multiset":
    ...
    def forward(self, x, edge_index, edge_attr, problemType, batch):
        x = self.encode_graph(x, edge_index, edge_attr, batch)

        # Keep the graph dimension intact.  The original implementation flattened
        # every graph into one row, which silently restricted training to batch=1.
        x = x.reshape(x.size(0), -1)
        problemType = problemType.reshape(-1, 1).to(device=x.device, dtype=x.dtype)
        if problemType.size(0) != x.size(0):
            raise ValueError(
                f"problemType batch mismatch: graphs={x.size(0)}, "
                f"problemTypes={problemType.size(0)}"
            )
